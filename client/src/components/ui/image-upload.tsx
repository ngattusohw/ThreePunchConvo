...
const ImageUpload: React.FC<ImageUploadProps> = ({
  ...
  const uploadRules =
    rules ||
    (variant === "profile" ? PROFILE_IMAGE_RULES : DEFAULT_IMAGE_RULES);
    ...
    <div className={`relative ${className}`}>
      ...
      <div
        ...
        className={`cursor-pointer rounded-lg border-2 border-dashed p-6 text-center transition-colors ${
          disabled
            ? "cursor-not-allowed border-gray-600 bg-gray-800"
              ...
              {!isValidating && !isCompressing && (
                <>
                  ...
                  <p className='text-xs text-gray-400'>
                    {getAcceptedFormats()} up to{" "}
                    {formatFileSize(uploadRules.maxFileSize)}
                    {multiple &&
                      uploadRules.maxFilesPerUpload > 1 &&
      ...
      {validationErrors.length > 0 && (
        <div className='mt-2 rounded-lg border border-red-500/30 bg-red-900/20 p-3'>
      ...
      {validationWarnings.length > 0 && (
        <div className='mt-2 rounded-lg border border-yellow-500/30 bg-yellow-900/20 p-3'>
          <div className='flex items-start'>
            <svg
      ...
      <div className='mt-2 text-xs text-gray-500'>
        ...
        <p>
          Max dimensions: {uploadRules.maxWidth} × {uploadRules.maxHeight}px
        </p>
