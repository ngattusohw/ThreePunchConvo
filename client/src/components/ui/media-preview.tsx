...
const MediaPreview = ({ media, threadTitle }: MediaPreviewProps) => {
  ...
  const handleError = (e: React.SyntheticEvent<HTMLImageElement>) => {
    ...
    getToken().then(token => {
      ...
      .then(response => {
        console.log('Fetch response for', media.url, ':', response.status, response.statusText);
        if (!response.ok) {
  ...
  React.useEffect(() => {
    ...
    getToken().then(token => {
      ...
      .then(response => {
        ...
        if (!response.ok) {
          console.warn('Image may not be accessible:', response.status, response.statusText);
        }
      })
      .catch(error => {
  ...
  const isSmall = naturalDimensions.width < 200 && naturalDimensions.height < 200;
