...
export async function validateImageFile(
  ...
  if (!rules.allowedFormats.includes(file.type)) {
    errors.push(`File format "${file.type}" is not allowed. Supported formats: ${rules.allowedFormats.join(', ')}`);
  }

  // Check file size
  if (file.size > rules.maxFileSize) {
    errors.push(`File size (${formatFileSize(file.size)}) exceeds maximum allowed size of ${formatFileSize(rules.maxFileSize)}`);
  }
  ...
  if (file.type.startsWith('image/')) {
    try {
      ...
      if (dimensions.width > rules.maxWidth || dimensions.height > rules.maxHeight) {
        errors.push(`Image dimensions (${dimensions.width}×${dimensions.height}) exceed maximum allowed size of ${rules.maxWidth}×${rules.maxHeight}`);
      }
      
      if (dimensions.width < rules.minWidth || dimensions.height < rules.minHeight) {
        errors.push(`Image dimensions (${dimensions.width}×${dimensions.height}) are below minimum required size of ${rules.minWidth}×${rules.minHeight}`);
      }

      // Add warnings for edge cases
      const aspectRatio = dimensions.width / dimensions.height;
      if (aspectRatio > 5 || aspectRatio < 0.2) {
        warnings.push('Image has an extreme aspect ratio and may not display well');
      }

      if (dimensions.width > 2048 || dimensions.height > 2048) {
        warnings.push('Large image may take longer to load and use more bandwidth');
      }
  ...
  if (suspiciousExtensions.some(ext => fileName.includes(ext))) {
    errors.push('File appears to contain executable content and cannot be uploaded');
  }
...
export async function validateImageFiles(
  ...
  if (fileArray.length > rules.maxFilesPerUpload) {
    totalErrors.push(`Too many files. Maximum ${rules.maxFilesPerUpload} files allowed`);
  }

  // Check total size
  const totalSize = fileArray.reduce((sum, file) => sum + file.size, 0);
  if (totalSize > rules.totalMaxSize) {
    totalErrors.push(`Total file size (${formatFileSize(totalSize)}) exceeds maximum allowed size of ${formatFileSize(rules.totalMaxSize)}`);
  }
  ...
  const hasAnyErrors = results.some(result => !result.isValid) || totalErrors.length > 0;
 ...
 */
export function performSecurityCheck(file: File): { isSafe: boolean; issues: string[] } {
  const issues: string[] = [];
